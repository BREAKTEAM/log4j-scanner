...
var online = flag.Bool("online", false, "run offline of online")
var server = flag.String("server", "", "ip of server online")
var file, err = os.Create("results.txt")
var nett net.Conn

func Internal_hi()  {
	fmt.Printf("internal_hi")
}

func Internal_Execute(include_zip bool, mode *string, server *string, online *bool) {
	flag.Parse()
	target := flag.Arg(1)
	// Parse the arguments and flags provided to the program.
	if *online {
		nett, err = net.Dial("tcp", *server+":8080")
		if err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
		r := bufio.NewReader(nett)
	...
	dat, err := os.ReadFile("results.txt")
	if err != nil {
		log.Fatal(err)
	}
	if *online {
